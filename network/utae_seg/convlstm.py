    ...
    def forward(self, input_tensor, hidden_state=None, pad_mask=None):
        ...
        if hidden_state is not None:
            ...
            hidden_state = self._init_hidden(
                batch_size=input_tensor.size(0), input_size = (input_tensor.shape[3], input_tensor.shape[4]), device=input_tensor.device
            )
...
class ConvLSTM_Seg(nn.Module):
    def __init__(
        self, num_classes, input_dim, hidden_dim, kernel_size, pad_value=0
    ):
